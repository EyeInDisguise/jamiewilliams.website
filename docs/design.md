...
The homepage follows the real controller signal path as the reader scrolls. A sticky readout changes at each factual step, and a thin progress line advances with the page. The steps stay readable without scripting; reduced-motion users get a normal static list. A crisp pointer reticle is an extra layer beside the native cursor on fine pointers. It disappears on touch devices and for reduced-motion users.
