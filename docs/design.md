...
The homepage follows the real controller signal path as the reader scrolls. A sticky readout changes at each factual step, and a thin progress line advances with the page. The steps stay readable without scripting; reduced-motion users get a normal static list. A small signal-probe cursor replaces the native pointer on fine pointers, with a lime light that follows behind the content. It expands over links and buttons and compresses on click. Touch devices and reduced-motion users keep the normal pointer.
